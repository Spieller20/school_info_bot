...
def func(message):
    ...
    #Меню выбранной даты урока
     ...
     bot.send_message(message.chat.id,
                      ...
                      "\n\n<b><i>Музыкальная литература:</i></b> Класса нет\n",
                         parse_mode='HTML')
     bot.send_photo(message.chat.id, 'https://disk.yandex.ru/i/C-G_7LJ4x_q6ow', caption='6 и 7 классы: диктант транспонировать в Тональность с 3 знаками (на выбор - либо ля мажор, либо ми бемоль мажор)')
     bot.send_photo(message.chat.id, 'https://disk.yandex.ru/i/vDeoyZpoboVpdQ', caption='1 кл. внебюджет (7-9 лет): нарисовать музыкальную лесенку (на фото примеры того, как она может выглядеть), подписать ступеньки (как в классе)')
     bot.send_message(message.chat.id, "1 кл. внебюджет (10-14 лет): Построить в тетради от до, ми, соль, си полутон вверх. \nОт нот ре, фа, ля построить тон вверх")   
